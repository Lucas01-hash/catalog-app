...
import { CancelDemandService } from 'src/demand/services/cancel-demand/cancel-demand.service';

@ApiTags('Demand')
@Controller('api/v1/demand')
export class CancelDemandController {
  constructor(private readonly cancelDemand: CancelDemandService) {}

  @Put('/:id/cancel-demand')
  @ApiOperation({ summary: 'Cancel Demand' })
  @ApiResponse({
    status: 200,
    description: 'Sucess.',
    schema: {
  ...
  async create(@Param('id') id: string): Promise<Demand> {
    return await this.cancelDemand.cancel(id);
  }
}
